...
const Header = ({ detailData, contactData, error }) => {
  console.log("detaildata under header", detailData.given_name, detailData.session_valid);
  console.log("contactData under header", contactData);
  ...
  useEffect(() => {
    // Use a safe check for window object and fetch the selected account from localStorage
    if (typeof window !== 'undefined') {
      setSelectedAccount(localStorage.getItem('selectedAccount'));
      //const [selectedAccount, setSelectedAccount] = useState(key);
    }
        ...
        <div className='navSection'>
          <nav>
            <ul className='bottom-links'>
              ...
              <li><Link href="#">Support</Link></li>
              <li><Link href="#">News Events</Link></li>
              <li onClick={ToggleSearchBox}><Link href="#"><Image src={Magify} alt="Honeywell search icon" className='search-icon' /></Link>
                {toggleSearchBox && <SearchBar setToggleSearchBox={setToggleSearchBox}></SearchBar>
              }
              </li>
            </ul>
          </nav>
